...
const (
	POST_INDEX  = "post"
	DISTANCE    = "200km"
	BUCKET_NAME = "around_bucket_970912"
...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	// front end need the access control, visit from different domain
	// "*"  represents open to all visit
	...
	// Save image to GCS
	// post has txt value and file
	// FoarmValue and FormFile
	...
	//use uuid to new a different id
	id := uuid.New()
	...
	if p.Type == "image" {
		//Sprintf give the print content to uri
...
func handlerSearch(w http.ResponseWriter, r *http.Request) {
	...
	// if use panic will stop the server here we just return error to handle
	if err != nil {
		// http.StatusInternalServerError means error 500
	...
	// convert post form data to json format
	if err != nil {
		http.Error(w, "Failed to read parse the result from Elasticsearch", http.StatusInternalServerError)
	}
	w.Write(js)
}
...
func saveToGCS(r io.Reader, objectName string) (string, error) {
	...
	// access control anyone can read
